	 ...
	 // Class section
       ...
       $formatted_date=date_format($dateTime,'Y-m-d' );
		   ...
		   $community=$this->input->post('community');
			 ...
			 $total_year_of_experience=$this->input->post('total_year_of_experience');
			 $skillsets=$this->input->post('skillsets');
			 $user_file = $_FILES["user_resume"]["name"];
			 if(empty($user_file)){
					$user_resume=' ';
				}else{
					$temp = pathinfo($user_file, PATHINFO_EXTENSION);
					$user_resume = round(microtime(true)) . '.' . $temp;
					$uploaddir_file = 'assets/staff/resume/';
 				 	$user_resume_file = $uploaddir_file.$user_resume;
 					move_uploaded_file($_FILES['user_resume']['tmp_name'], $user_resume_file);
				}
			 ...
			 $profilepic = $uploaddir.$userFileName;
				...
				$datas=$this->teachermodel->teacher_create($role_type_id,$name,$email,$sec_email,$sex,$formatted_date,$age,$nationality,$religion,$community_class,$community,$mobile,$sec_phone,$address,$class_teacher,$class_name,$subject,$multiple_sub,$qualification,$groups_id,$activity_id,$status,$user_id,$userFileName,$previous_institute,$total_year_of_experience,$skillsets,$user_resume);
       ...
       $dob=date_format($dateTime,'Y-m-d' );
		   ...
		   $community=$this->input->post('community');
			 ...
			 $userFileName =time().'-'.$teacher_pic;
				...
				$total_year_of_experience=$this->input->post('total_year_of_experience');
				$skillsets=$this->input->post('skillsets');
				$old_user_resume=$this->input->post('old_user_resume');
				$user_file = $_FILES['user_resume']['name'];
				if(empty($user_file)){
					$user_resume=$old_user_resume;
				}else{
					$temp = pathinfo($user_file, PATHINFO_EXTENSION);
					$user_resume = round(microtime(true)) . '.' . $temp;
					$uploaddir = 'assets/staff/resume/';
					$user_resume_file = $uploaddir.$user_resume;
					move_uploaded_file($_FILES['staff_new_pic']['tmp_name'], $user_resume_file);
				}

				$datas=$this->teachermodel->save_teacher($role_type_id,$name,$email,$sec_email,$sex,$dob,$age,$nationality,$religion,$community_class,$community,$mobile,$sec_phone,$address,$userFileName,$class_teacher,$class_name,$subject,$multiple_sub,$qualification,$groups_id,$activity_id,$status,$user_id,$teacher_id,$previous_institute,$total_year_of_experience,$skillsets,$user_resume);
