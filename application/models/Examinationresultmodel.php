   ...
   /*    function getall_subname($user_id,$cls_masid,$exam_id,$user_type)
    ...
    function getall_stuname($user_id, $cls_masid, $exam_id)
    {
        ...
        //$sql="SELECT t.teacher_id,t.class_teacher,t.name,t.subject,en.enroll_id,en.name,en.admisn_no,en.class_id FROM edu_teachers AS t,edu_enrollment AS en WHERE t.teacher_id='$teacher_id' AND en.class_id='$cls_masid'";
         ...
         $sql="SELECT eed.is_internal_external AS is_ei,en.enroll_id,en.name,en.admission_id,en.admisn_no,en.class_id,m.subject_id,m.classmaster_id,m.internal_mark,m.internal_grade,m.external_mark,m.external_grade,m.total_marks,m.total_grade,a.admission_id,a.admisn_no,a.name,a.sex,a.language,s.subject_name
FROM edu_exam_details AS eed,edu_enrollment AS en,edu_exam_marks AS m,edu_admission AS a LEFT JOIN edu_subject AS s ON s.subject_id=a.language WHERE en.class_id='$cls_masid' AND en.enroll_id=m.stu_id
AND m.exam_id='$exam_id' AND en.admission_id=a.admission_id AND  eed.exam_id='$exam_id' AND eed.classmaster_id='$cls_masid' AND eed.subject_id=m.subject_id ORDER BY a.sex DESC,en.name ASC";
