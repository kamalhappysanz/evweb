...
//DELETE SECTION NAME
       ...
       function get_subject_to_class($class_id){
         $query="SELECT estc.subject_id,es.subject_name FROM edu_subject_to_class AS estc LEFT JOIN
         edu_subject  AS es  ON estc.subject_id=es.subject_id WHERE estc.class_master_id='$class_id' AND estc.status='Active'";
         $resultset=$this->db->query($query);
