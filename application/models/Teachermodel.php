...
//CREATE ADMISSION
        ...
        function teacher_create($role_type_id,$name,$email,$sec_email,$sex,$formatted_date,$age,$nationality,$religion,$community_class,$community,$mobile,$sec_phone,$address,$class_teacher,$class_name,$subject,$multiple_sub,$qualification,$groups_id,$activity_id,$status,$user_id,$userFileName,$previous_institute,$total_year_of_experience,$skillsets,$user_resume)
		{
          $check_email="SELECT * FROM edu_teachers WHERE email='$email'";
          $result=$this->db->query($check_email);
          if($result->num_rows()==0){
         $query="INSERT INTO edu_teachers(role_type_id,name,email,sec_email,sex,dob,age,nationality,religion,community_class,community,phone,sec_phone,address,class_teacher,class_name,subject,subject_handling,qualification,house_id,extra_curicullar_id,profile_pic,created_by,created_at,status,previous_institute,total_year_of_experience,skillsets,user_resume) VALUES ('$role_type_id','$name','$email','$sec_email','$sex','$formatted_date','$age','$nationality','$religion','$community_class','$community','$mobile','$sec_phone','$address','$class_teacher','$class_name','$subject','$multiple_sub','$qualification','$groups_id','$activity_id','$userFileName','$user_id',NOW(),'$status','$previous_institute','$total_year_of_experience','$skillsets','$user_resume')";
      ...
      function get_subject_handling_teacher($id){
       ...
       function save_teacher($role_type_id,$name,$email,$sec_email,$sex,$dob,$age,$nationality,$religion,$community_class,$community,$mobile,$sec_phone,$address,$userFileName,$class_teacher,$class_name,$subject,$multiple_sub,$qualification,$groups_id,$activity_id,$status,$user_id,$teacher_id,$previous_institute,$total_year_of_experience,$skillsets,$user_resume)
	   {
            $query="UPDATE edu_teachers SET role_type_id='$role_type_id',name='$name',email='$email',sec_email='$sec_email',sex='$sex',dob='$dob',age='$age',nationality='$nationality',religion='$religion',community_class='$community_class',community='$community',phone='$mobile',sec_phone='$sec_phone',address='$address',profile_pic='$userFileName',class_teacher='$class_teacher',class_name='$class_name',subject='$subject',subject_handling='$multiple_sub',qualification='$qualification',house_id='$groups_id',extra_curicullar_id='$activity_id',status='$status',update_at=NOW(),updated_by='$user_id',previous_institute='$previous_institute',total_year_of_experience='$total_year_of_experience',skillsets='$skillsets',user_resume='$user_resume' WHERE teacher_id='$teacher_id'";
             $res=$this->db->query($query);
